...
InputParameters validParams<InterfaceDiffusion>()
{
  ...
  params.addParam<bool>(
      "use_variable_side_material", true,
  ...
  params.addParam<bool>(
      "use_neighbor_side_material", true,
...
InterfaceDiffusion::InterfaceDiffusion(const InputParameters &parameters)
  : InterfaceKernel(parameters),
    ...
    _variable_diffusivity_input(
        getParam<std::string>("variable_side_diffusivity")),
    _neighbor_diffusivity_input(
        isParamValid("neighbor_side_diffusivity")
    ...
    _variable_diffusion_coefficient_property(
        _use_variable_side_material
            ? &getMaterialProperty<Real>(_variable_diffusivity_input)
            : NULL),
    ...
    _variable_diffusion_coefficient(
        _use_variable_side_material
            ? 0.0
            : atof(&_variable_diffusivity_input.front())),
...
Real InterfaceDiffusion::computeQpResidual(Moose::DGResidualType type)
{
  if (_use_variable_side_material)
    _variable_diffusion_coefficient =
        (*_variable_diffusion_coefficient_property)[_qp];
  if (_use_neighbor_side_material)
    _neighbor_diffusion_coefficient =
        (*_neighbor_diffusion_coefficient_property)[_qp];

  Real r =
      0.5 * (-_variable_diffusion_coefficient * _grad_u[_qp] * _normals[_qp] +
             -_neighbor_diffusion_coefficient * _grad_neighbor_value[_qp] *
                 _normals[_qp]);
...
Real InterfaceDiffusion::computeQpJacobian(Moose::DGJacobianType type)
{
  Real jac = 0;
  ...
  case Moose::ElementElement:
    jac -= 0.5 * _variable_diffusion_coefficient * _grad_phi[_j][_qp] *
           _normals[_qp] * _test[_i][_qp];
    break;

  case Moose::NeighborNeighbor:
    jac += 0.5 * _neighbor_diffusion_coefficient * _grad_phi_neighbor[_j][_qp] *
