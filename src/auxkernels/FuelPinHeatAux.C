...
FuelPinHeatAux::FuelPinHeatAux(const InputParameters & parameters)
...
FuelPinHeatAux::computeValue()
{
  ...
  const Real neutronic_heating = exp(-2.30258509299 * (_in[_qp] - _initial_temperature) / (_max_temperature - _initial_temperature)); // 10% heat generation at _max_temperature

  return _max_heating_rate * neutronic_heating * axial_factor * radial_factor;
}
