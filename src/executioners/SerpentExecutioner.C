                                         ...
                                         : 1.0)))
{
  ...
  std::vector<Real> seed_coefficients(getCoefficients().size(), 0.0);
...
void
SerpentExecutioner::generateSerpentInterfaceFile(const std::string & interface_base,
                                                 const std::vector<Real> & out_coefficients) const
{
  ...
  else
  {
    ...
    bool first = all_zero;
    for (const auto & coefficient : out_coefficients)
      writer << (first ? first = false, _serpent_initial_constant_fe_value : coefficient) << "\n";
  }
